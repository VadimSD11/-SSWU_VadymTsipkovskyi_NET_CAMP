...
namespace Home_task_9
{
    public class Kitchen
    {
        ...
        public Kitchen(Chef[] pizzaChefs, Chef[] drinksChefs, Chef[] sweetsChefs)
        {
        ...
        public void SetChefsEventConsoleOutput() {
            foreach (var item in _pizzaChefs)
            {
                item.DishPreparationEvent+= item.DishPreparationHandlerForConsole;
                item.DishFinishedEvent+= item.DishFinishedHandlerForConsole;
            }
            foreach (var item in _drinksChefs)
            {
                item.DishPreparationEvent += item.DishPreparationHandlerForConsole;
                item.DishFinishedEvent += item.DishFinishedHandlerForConsole;

            }
            foreach (var item in _sweetsChefs)
            {
                item.DishPreparationEvent += item.DishPreparationHandlerForConsole;
                item.DishFinishedEvent += item.DishFinishedHandlerForConsole;
